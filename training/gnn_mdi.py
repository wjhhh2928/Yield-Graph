...
def train_gnn_mdi(data, args, log_path, device=torch.device('cpu')):
    ...
    for epoch in range(args.epochs):
        ...
        if args.loss_mode == 1:
            ...
            label_train = train_labels[known_mask]
            pred_train = pred_train[known_mask]
        else:
            label_train = train_labels[:int(train_edge_attr.shape[0] / 2)]
        ...
        with torch.no_grad():
            ...
            if hasattr(args,'ce_loss') and args.ce_loss:
                ...
                label_test = test_labels[:int(test_edge_attr.shape[0] / 2)]###
