...
class GNNStack(torch.nn.Module):
    def __init__(self, 
        ...
        self.input_node_encoder = nn.Linear(node_input_dim, node_dim)
        self.input_edge_encoder = nn.Linear(edge_input_dim, edge_dim)
    ...
    def update_edge_attr(self, x, edge_attr, edge_index, mlp):
        x_i = x[edge_index[0], :]   # [E, node_dim]
        x_j = x[edge_index[1], :]   # [E, node_dim]
        ...
        input_feature = torch.cat((x_i, x_j, edge_attr), dim=-1)  # [E, 2*node_dim + edge_attr_dim]
        edge_attr = mlp(input_feature)
    ...
    def forward(self, x, edge_attr, edge_index):
        # 
        x = self.input_node_encoder(x)           # [N, node_dim]
        if edge_attr.dim() == 1:
            edge_attr = edge_attr.unsqueeze(1)  # [E] -> [E, 1]
        elif edge_attr.shape[0] == 1 and edge_attr.shape[1] != 1:
            edge_attr = edge_attr.T  # [1, E] -> [E, 1]

        edge_attr = self.input_edge_encoder(edge_attr)  # [E, edge_dim]
