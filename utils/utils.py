...
# def get_known_mask(edge_num):
#     # known_mask = (torch.FloatTensor(edge_num, 1).uniform_() < known_prob).view(-1)
#     known_mask = (edge_num != -0.001).view(-1)
#     return known_mask
# def get_known_mask2(known_prob, edge_num):
#     known_mask = (torch.FloatTensor(edge_num, 1).uniform_() < known_prob).view(-1)
#     return known_mask
def get_known_mask(prob, length):
    return (torch.rand(length) < prob)
